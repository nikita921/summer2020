...
{
    // sort s according to the ordering relationship  
    // passed in as the parameter comp

    for (int i = 1; i != s.size(); i++)
    {
        Sensor sort = s[i];
        int x = i;
        while (x > 0 && !comp(s[x - 1], sort))
        {
            s[x] = s[x - 1];
            x--;
        }
...
int main()
{
    ...
    // destruction time.
    {
        ...
        for (size_t k = 0; k < ids.size(); k++) 
        {
            sensorPtr.push_back(&auxSensors[k]);
        }
